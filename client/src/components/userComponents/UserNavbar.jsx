...
const UserNavbar = ({ onMenuToggle }) => {
  const { logout, user } = useSafeCity();
  const [showNotifications, setShowNotifications] = useState(false);
  const [showProfileMenu, setShowProfileMenu] = useState(false);

  // Mock notifications data
  const notifications = [
    { id: 1, message: "New incident reported in your area", time: "5 min ago", read: false, type: "incident" },
    { id: 2, message: "Safety alert: Road closure", time: "10 min ago", read: false, type: "alert" },
    { id: 3, message: "Weekly safety report available", time: "1 hour ago", read: true, type: "report" },
    { id: 4, message: "Community meeting reminder", time: "2 hours ago", read: true, type: "community" },
  ];
  ...
  const getNotificationIcon = (type) => {
    switch(type) {
      case 'incident': return <FaCircle className="text-red-500 text-xs" />;
      case 'alert': return <FaBell className="text-orange-500" />;
      case 'report': return <FaCog className="text-blue-500" />;
      case 'community': return <FaComments className="text-green-500" />;
        ...
        <div className="flex items-center gap-3">
                  ...
                  {notifications.map((notification) => (
                    <div
                      ...
                      className={`p-3 border-b border-gray-600 hover:bg-safecity-dark cursor-pointer transition-colors ${
                        !notification.read ? 'bg-red-900/20 border-l-4 border-l-red-500' : ''
                      }`}
                    >
                      <div className="flex gap-3 items-start">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-safecity-text">{notification.message}</p>
                          <p className="text-xs text-safecity-muted mt-1">{notification.time}</p>
                        </div>
                        {!notification.read && (
          ...
          <div className="relative">
            ...
            >
              ...
              <FaChevronDown className={`h-3 w-3 text-safecity-muted transition-transform duration-200 ${
